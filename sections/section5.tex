...
\item 另一方面，其实相比于~\eqref{eq:5-2}，式~\eqref{eq:5-4} 的范围其实是扩大了，因为对任意满足 $0\in\mathrm{int}L$ 的凸体 $L$ 都有 $L=[h_L]$ (命题~\ref{prop:wulff_eq})。同时 $h_L\in\mathcal{C}^+\big(\mathbb S^{n-1}\big)$ 总是成立的。由于条件放松，有 $\widetilde{\theta\,}\le \theta$。
\end{itemize}
